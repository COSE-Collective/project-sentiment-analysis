...
def data_preprocessing(data):
    ...
    data = pd.DataFrame({"Tweet": data[5],
                         "Sentiment": data[0]})
    data.dropna(inplace=True)
    data = data.drop_duplicates(keep='first')
    step += 1
    print('Step {}/{}: Converting categorical data into integer format'.format(step, num_steps))
    data["Sentiment"] = np.array(list(map(lambda y: 2 if y == 4 else (1 if y == 2 else (0 if y == 0 else 3)), data["Sentiment"])))
    step += 1
...
if __name__ == '__main__':
    ...
    test_data3 = pd.read_csv('../../datasets/twitter_test3.csv', encoding='latin1', header=None)
    test_data3 = data_preprocessing(test_data3)
    print('Data saving')
    test_data3.to_csv('../../datasets/prepared_test3.csv', encoding='utf-8', index=False)
    print('Completed')
