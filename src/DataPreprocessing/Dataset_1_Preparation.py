...
def remove_emoji(i):
    for token in i:
        if token in EMOJI_DATA:
            i = re.sub(r'(' + token + ')',
                       " ".join(emoji.demojize(token).replace(",", "").replace(":", "").replace("_", " ").split()), i)
    return i
...
def data_preprocessing(data):
    step = 1
    num_steps = 13
    print('Step {}/{}: Renaming columns and removing "irrelevant" sentiments'.format(step, num_steps))
...
if __name__ == '__main__':
    ...
    test_data = pd.read_csv('../../datasets/twitter_test1.csv', header=None)
    train_data = data_preprocessing(train_data)
    test_data = data_preprocessing(test_data)
    print('Data saving')
    train_data.to_csv('../../datasets/prepared_train1.csv', encoding='utf-8', index=False)
    test_data.to_csv('../../datasets/prepared_test1.csv', encoding='utf-8', index=False)
    print('Completed')
